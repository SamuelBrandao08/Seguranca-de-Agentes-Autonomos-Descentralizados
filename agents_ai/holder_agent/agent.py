...
async def chat_endpoint(inp: ChatInput):
    ...
    try:
        if func == "setup_telco":
            ...
            STATE.update(state)
            print("ESTADO DA CONEXAO NO CLLINTE", STATE)
            ...
            url = acapy_controller.CLIENTE_ADMIN
            print("ID da conexao e url: ", STATE['conn_id_client'], url)
            
            conn_id = STATE['conn_id_client']
            msg = "Por que o céu é azul?"
            if not conn_id: return "O ID da conexao nao foi encontrado." 
            result = await acapy_controller.enviar_mensagem(session, url, conn_id, msg) # connection_id?
...
async def on_message(request: Message):
    data = await request.json()
    ...
    logging.info(f"Você recebeu uma mensagem segura DIDComm: {content}")
